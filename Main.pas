...
type
  ...
  private
    ...
    FInst: TSQLInstall; // Declaração da instância de TSQLInstall
  public
    { Public declarations }
    property Inst: TSQLInstall read FInst; // Propriedade para acessar a instância de TSQLInstall
  end;
...
begin
  ...
  try
    ...
    on E: Exception do
    begin
      ShowMessage('Erro ao abrir o formulário de atualização: ' + E.Message);
      FreeAndNil(FAttEstr); // Libera a memória se ocorrer um erro
    end;
  end;
end;
...
begin
  ...
  FLogin.ShowModal;
  // Criação da instância de TSQLInstall
  FInst := TSQLInstall.Create(Self);
  FInst.TypeServer := tsOracle; // Defina o tipo de servidor, se necessário
end;
